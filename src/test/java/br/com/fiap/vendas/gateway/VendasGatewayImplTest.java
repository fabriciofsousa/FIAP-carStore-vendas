...
class VendasGatewayImplTest {
    ...
    private VendasDynamoRepository VendasDynamoRepository;
    private VendasGatewayImpl vendasGateway;

    @BeforeEach
    void setUp() {
        VendasDynamoRepository = mock(VendasDynamoRepository.class);
        vendasGateway = new VendasGatewayImpl(VendasDynamoRepository);
    }
    ...
    void salvar_deveChamarRepositoryESalvar() {
        ...
        when(VendasDynamoRepository.save(any(VendasEntity.class))).thenReturn(savedEntity);

        Vendas result = vendasGateway.salvar(venda);

        assertNotNull(result.getId());
        assertEquals(venda.getClienteId(), result.getClienteId());
        verify(VendasDynamoRepository, times(1)).save(any(VendasEntity.class));
    }
    ...
    void buscarPorId_deveRetornarVendaQuandoEncontrada() {
        ...
        when(VendasDynamoRepository.findById(id)).thenReturn(Optional.of(entity));

        Optional<Vendas> result = vendasGateway.buscarPorId(id);

        assertTrue(result.isPresent());
        assertEquals(entity.getId(), result.get().getId());
        verify(VendasDynamoRepository, times(1)).findById(id);
    }

    @Test
    void buscarPorId_deveRetornarVazioQuandoNaoEncontrada() {
        UUID id = UUID.randomUUID();
        when(VendasDynamoRepository.findById(id)).thenReturn(Optional.empty());
    ...
    void buscarVendidosOrdenadosPorPreco_deveRetornarListaDeVendas() {
        ...
        verify(VendasDynamoRepository, times(1))
    ...
    void buscarVendaIniciadaPorVeiculo_deveRetornarVendaQuandoExistir() {
        ...
        when(VendasDynamoRepository.findByVeiculoIdAndStatus(veiculoId, Status.INICIADA))
